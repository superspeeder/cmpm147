...
function setup() {
  ...
  let input_span = createSpan();
  input_span.style("display: flex; align-items: center; gap: 10px; margin-top: 10px;")
  input_span.parent("container");
  
  let label = createP();
  label.html("World key: ");
  label.parent(input_span);
  ...
  let button = createButton("Reset Changes");
  button.parent(input_span)
  button.mousePressed(() => {if (window.p3_resetChanges) { window.p3_resetChanges(); }});

  createP("Arrow keys scroll. Clicking raises the height at a tile. Shift-clicking lowers the height.").parent("container");
...
function draw() {
  ...
  let overdraw = 0.1;
  let bottom_overdraw = 1;

  let y0 = Math.floor((0 - overdraw) * tile_rows);
  let y1 = Math.floor((1 + bottom_overdraw) * tile_rows);
  let x0 = Math.floor((0 - overdraw) * tile_columns);
  let x1 = Math.floor((1 + overdraw) * tile_columns);
  
  let [mouse_i, mouse_j] = worldToScreen(
    world_pos,
    [camera_offset.x, camera_offset.y]
  );
  ...
  for (let y = y0; y < y1; y++) {
    ...
    for (let x = x0; x < x1; x++) {
      drawTile(
        ...
        [camera_offset.x, camera_offset.y], world_pos
...
function drawTile([world_x, world_y], [camera_x, camera_y], [mouse_i, mouse_j]) {
  let [screen_x, screen_y] = worldToScreen(
    [world_x, world_y],
    [camera_x, camera_y]
  );
  ...
  if (window.p3_drawTile) {
    window.p3_drawTile(world_x, world_y, mouse_i, mouse_j, -screen_x, screen_y);
  }
  pop();
}
