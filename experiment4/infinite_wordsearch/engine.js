...
function draw() {
  ...
  for (let y = y0; y < y1; y++) {
    ...
    for (let y = y0; y < y1; y++) {
    for (let x = x0; x < x1; x++) {
      drawTile2(tileRenderingOrder([x + world_offset.x, y - world_offset.y]), [
        camera_offset.x,
        camera_offset.y
      ]); // odd row
    }
...
// Draw a tile, mostly by calling the user's drawing code.
function drawTile2([world_x, world_y], [camera_x, camera_y]) {
  let [screen_x, screen_y] = worldToScreen(
    [world_x, world_y],
    [camera_x, camera_y]
  );
  push();
  translate(0 - screen_x, screen_y);
  if (window.p3_drawTile2) {
    window.p3_drawTile2(world_x, world_y, -screen_x, screen_y);
  }
  pop();
}
